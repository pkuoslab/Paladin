...
class adb{
	static async devices(options){
		...
		let {timeout} = options; if (timeout === void 0) timeout = DEFAULT.devices.timeout;
	...
	async forward(params, options){
		...
		if (options === void 0) options = {};
		let {timeout} = options; if (timeout === void 0) timeout = DEFAULT.forward.timeout;

		let args = [];
		if (params.remove_all)
				args.push("--remove-all");
		  else	args.push(params.local, params.remote);
	...
	async ps(params, options){
		if (params === void 0) params = {};
		if (options === void 0) options = {};
		let {timeout} = options; if (timeout === void 0) timeout = DEFAULT.ps.timeout;
...
//			"user": proc[0],
			"pid": parseInt(proc[1]),
//			"ppid": parseInt(proc[2]),
//			"vsize": parseInt(proc[3]),
//			"rss": parseInt(proc[4]),
//			"wchan": proc[5],
//			"pc": parseInt(proc[6], 16),
			"name": proc[8]
		}));
		return procs;
	}
	...
	async dumpsys(params, options){
		if (params === void 0) params = {};
		if (options === void 0) options = {};
		let {timeout} = options; if (timeout === void 0) timeout = DEFAULT.dumpsys.timeout;
	...
	async screenshot(params, options){
		...
		if (options === void 0) options = {};
		let {timeout} = options; if (timeout === void 0) timeout = DEFAULT.screenshot.timeout;
	...
	async pull(params, options){
		...
		if (options === void 0) options = {};
		let {timeout} = options; if (timeout === void 0) timeout = DEFAULT.pull.timeout;
	...
	async unix_domain_sockets(options){
		if (options === void 0) options = {};
		let {timeout} = options; if (timeout === void 0) timeout = DEFAULT.cat.timeout;

		let msg = await this.shell("cat", ["/proc/net/unix"], {timeout});

		let lines = msg.trim().split("\n");
		let headers = lines.shift();
		// may headers be useful...
...
//				"Inode": frags[6],
	...
	async input(params, options){
		...
		let {timeout} = options;
		if (timeout === void 0) timeout = DEFAULT.input.timeout;
	...
	async wm(params, options){
		...
		let {timeout} = options;
		if (timeout === void 0) timeout = DEFAULT.wm.timeout;
	...
	async getevent(params, options){
		...
		let args = [];
		let {timeout} = options;
		if (timeout === void 0) timeout = DEFAULT.getevent.timeout;

		if (params.plain_text) args.push("-l");
		if (params.timestamps) args.push("-t");
		if (params.possible_events)	args.push("-p");
	...
	async monkey(params, options){
		...
		let {timeout} = options;
		if (timeout === void 0) timeout = DEFAULT.monkey.timeout;
	...
	async am(params, options){
		...
		let {timeout} = options;
		if (timeout === void 0) timeout = DEFAULT.am.timeout;
	...
	async logcat(params, options){
		...
		if (options === void 0) options = {};
		let {timeout} = options;
		if (timeout === void 0) timeout = DEFAULT.logcat.timeout;
	...
	ia_getevent(params){
		...
		let args = [];
		if (params.plain_text) args.push("-l");
		if (params.timestamp) args.push("-t");
	...
	ia_logcat(params){
		...
		if (params.filterspecs) args = args.concat(params.filterspecs);
