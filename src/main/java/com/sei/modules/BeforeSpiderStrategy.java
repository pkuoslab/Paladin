...
public class BeforeSpiderStrategy implements Strategy{
    ...
    static int MAX_SAM_TIME = 15;
    int sameTime;


    public BeforeSpiderStrategy(GraphAdjustor graphAdjustor, Map<String, Device> devices){
    ...
    public Decision make(String serial, ViewTree currentTree, ViewTree newTree, Decision prev_decision, int response){
        ...
        if(currentTree.getTreeStructureHash() == newTree.getTreeStructureHash()) {
            sameTime++;
            if(sameTime >= MAX_SAM_TIME) {
                sameTime = 0;
        ...
        CommonUtil.log("new tree:" + newTree.getActivityName() + "_" + newTree.getTreeStructureHash());
        ...
        if(device.targetActivity != null && newTree.getActivityName().equals(device.targetActivity)) {
            ...
            if (device.fragmentStack.getSize() > 1) {
                ...
                CommonUtil.log("1 To:" + device.fragmentStack.top().getSignature());
        ...
        if(newTree.hasWebview || newTree.getActivityName().toLowerCase().contains("login") || newTree.getActivityName().toLowerCase().contains("user")) {
            CommonUtil.log(newTree.getActivityName() + "_" + newTree.getTreeStructureHash() + " is over");
            //此节点已点击完毕，判断此节点是否是栈顶
            if (device.fragmentStack.getSize() > 1) {
                device.fragmentStack.pop();
                CommonUtil.log("2 To:" + device.fragmentStack.top().getSignature());
                return new Decision(Decision.CODE.GO, device.fragmentStack.top().getSignature());
            }else {
                custom(serial);
                return new Decision(Decision.CODE.STOP);
        ...
        //在栈中
        if (p != top){
            CommonUtil.log("3 To:" + device.fragmentStack.top().getSignature());
            return new Decision(Decision.CODE.GO, device.fragmentStack.top().getSignature());
        }
        ...
        if (device.fragmentStack.getSize() > 1) {
            device.fragmentStack.pop();
            CommonUtil.log("4 To:" + device.fragmentStack.top().getSignature());
            return new Decision(Decision.CODE.GO, device.fragmentStack.top().getSignature());
        }else {
            custom(serial);
            return new Decision(Decision.CODE.STOP);
